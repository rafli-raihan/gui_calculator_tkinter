...
def MyCalc(source, side):
    sObj = Frame(source, borderwidth=4, bd=4, bg='#818181')
    sObj.pack(side=side, expand=YES, fill=BOTH)
    return sObj

def tombol(source, side, text, command=None, bg="#c3c3c3"):
...
class app(Frame):
    def __init__(self):
        ...
        display = StringVar()
        Entry(self, relief=FLAT, textvariable=display, bd=30, bg="#010081", fg="#fdffff").pack(side=TOP, expand=YES, fill=BOTH)
        ...
        for TombolC in (['C']):
            ...
            for ichar in TombolC:
                tombol(ers, RIGHT, ichar, lambda sObj=display, q=ichar: sObj.set(''), bg='#ff0081')
        
        SamaDengan = MyCalc(self, RIGHT)
        for sd in '=':
            if sd == '=':
                TombolSama = tombol(SamaDengan, LEFT, sd, bg="#008080")
